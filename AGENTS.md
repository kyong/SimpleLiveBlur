...
物理カメラ
  └─ Python（face_blur_stream.py）
       ...
       ├─ MJPEGサーバー（localhost:8080、競合時は自動で空きポートへ）
       │     └─ OBS メディアソース → 配信
       └─ WebSocketサーバー（localhost:8081、オプション）
             └─ JSON経由で設定取得・変更・ステータス確認
```
...
| デーモンスレッド 2 | MJPEGサーバー（HTTP配信）+ WebSocket制御 |
...
| `history_frames` | `5` | スライダー（0〜15） | 検出マージフレーム数。0でOFF |
| `padding` | `0.25` | — | 顔領域の余白倍率 |
| `IMWRITE_JPEG_QUALITY` | `85` | — | MJPEG品質（1〜100） |
| `port` | `8080` | — | MJPEGサーバーのポート番号（競合時は自動で空きポートへ） |
...
| ポート競合 | 8080-8099を自動探索。全て使用中の場合はエラー終了 |
...
- [x] 検出漏れ対策として複数フレームの検出結果をマージ
- [x] WebSocket経由での制御（設定取得・変更・ステータス確認）
